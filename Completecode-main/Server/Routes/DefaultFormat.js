  ...
  fs.readFile(xmlFilePath, "utf-8", (err, xmlContent) => {
    ...
    parseString(xmlContent, { explicitArray: false }, (err, result) => {
      ...
      if (addOnInstructionDefinitionsNode) {
        ...
        const encodedDataNode = addOnInstructionDefinitionsNode.EncodedData;
        ...
        if (encodedDataNode) {
         ...
         // console.log("enoderdata", encodedData);
        }
      
        // Process the AddOnInstructionDefinitions
        const childrenData = addOnInstructionDefinitionsArray.map(
          (addOnInstructionDefinition) => ({
            name: addOnInstructionDefinition.$.Name,
          })
        );
      
      //  console.log("definition", childrenData);
      } else {
       // console.log("addOnInstructionDefinitionsNode or EncodedData not found");
      }
      
      const AOIIndex = controllerChildren.indexOf("AddOnInstructionDefinitions");
